...
namespace UltraStrore.Services
{
    public class GeminiServices : IGeminiServices   
    {
        ...
        public async Task<APIResponse> TraLoiLienHe(string userInput)
        {
            ...
            try
            {
                string Openning = "Bạn là nhân viên của một cửa hàng thời trang, hãy trả lời thắc mắc của khách hàng một cách lịch sự và chuyên nghiệp:\n";
                var GoogleAPIKey = _authSettings.Google.GoogleAPIKey;
                var GoogleAPIUrl = _authSettings.Google.GoogleAPIUrl;
        ...
        public async Task<string> TimKiemSanPham(string name)
        {
            ...
            if (data.Count > 0)
            {
                ...
                for (int i = 0; i < data.Count(); i++)
                {
                    ...
                    DataShow += $" Link Sản Phẩm: https://fashionhub.name.vn/product/{data[i].ID}";
                }
                try
                {
                    var GoogleAPIKey = _authSettings.Google.GoogleAPIKey;
